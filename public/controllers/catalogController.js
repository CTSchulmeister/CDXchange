...
app.get('/categories/:category/:item', (req, res) => {
    ...
    if(utilityFunctions.validateCategory(category) && req.session.theUser) {
        return viewItemAsUser(req, res, req.params.item, category);
    } else if(utilityFunctions.validateCategory(category)) {
        return viewItemAsPublic(req, res, req.params.item, category);
    } else {
 ...
 */
function viewItemAsUser(req, res, itemId, category) {
    let results = new Array();
    
    ItemModel.getItem(itemId)
        .then(item => {
            if(item.catalogCategory != category) {
                res.redirect('/404');
            }
 ...
 * @param {String} itemId - The id of the item.
 */
function viewItemAsPublic(req, res, itemId, category) {
    let results = new Array();
